...
char *DEVICE_NO = "DEVICE NO";
char *DEVICE_ROLE = "DEVICE ROLE";
char *DEVICE_NAME = "DEVICE NAME";
char *CONTAINER_SUPPORTED = "CONTAINER SUPPORTED?";
char *PKG_NAME = "IMAGE NAME";
char *PKG_STATUS = "IMAGE STATUS";
char *DOWNLOAD_CMD = "EXECUTE CMD";
char *TRIGGER = "TRIGGER";
char *STATUS = "CONTAINER STATUS";
char *PID = "CONTAINER ID";
char *KILL = "KILL";
char *PACKAGE_URI = "CONTAINER URI";
char *UPDATE = "UPDATE";
char *STATE = "UPDATE STATE";
char *UPDATE_RESULT  = "UPDATE RESULT";
char *DURATION = "UPDATE DURATION";
...
char *find = "/?appname=find-example";
char *insert = "/?appname=insert-example";
char *update = "/?appname=update-example";
char *delete = "/?appname=delete-example";
char *count = "/?appname=count-example";
...
lwm2m_object_t * get_object_device_update(int brokerNo, char* name,lwm2m_context_t * context, char *db_url)
{
    /*
    ...
    if (NULL != containerObj)
    {
		...
		for (i=0 ; i < NUM_INSTANCE ; i++){
			...
			if (strlen(db_url)!=0){
				...
				int mongo_ret = db_connection(db_url,collection);
				if (mongo_ret==0){
                                 ...
                                 lwm2m_object_t * objectP)
{
	...
	fprintf(stdout,"connect to the %s\n",db_addr);
	mongoc_collection_t *collection;
	int mongo_ret = db_connection(db_addr,collection);
	bson_t * query;
    ...
    do
    {
		...
		case RES_O_DEVICE_ROLE:
			...
			if (mongo_ret == 0){
				...
				doc_read_str(db_addr,query,DEVICE_ROLE,data->role);
			}
		...
		case RES_O_DEVICE_NAME:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,DEVICE_NAME,data->device_name);
			}
        ...
        case RES_O_CONTAINER_SUPPORTED:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,CONTAINER_SUPPORTED,data->container_supported);
			}
        ...
        case RES_O_PKG_NAME:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,PKG_NAME,data->image_name);
			}
        ...
        case RES_O_PKG_STATUS:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,PKG_STATUS,data->image_status);
			}
		...
		case RES_O_STATUS:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,STATUS,data->container_status);
			}
		...
		case RES_M_PID:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,PID,data->pid);
			}
        ...
        case RES_M_DOWNLOAD_CMD:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,DOWNLOAD_CMD,data->download_cmd);
			}
		...
		case RES_M_PACKAGE_URI:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_str(db_addr,query,PACKAGE_URI,data->package_url);
			}
		...
		case RES_M_STATE:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_int(db_addr,query,STATE,data->state);
			}
		...
		case RES_M_UPDATE_RESULT:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_int(db_addr,query,UPDATE_RESULT,data->result);
			}
		...
		case RES_M_DURATION:
			...
			if (mongo_ret == 0){
				query = bson_new ();
				BSON_APPEND_INT32(query, DEVICE_NO, data->device_no);
				doc_read_double(db_addr,query,DURATION,data->duration);
			}
                                    ...
                                    lwm2m_object_t * objectP)
{
	...
	case RES_O_TRIGGER:
		...
		if (strcmp(data->image_status,dimage_on_board)==0)
		{
			printf("\t\n Triggering Image \n\t");
