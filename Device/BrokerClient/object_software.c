...
#define RES_M_REGISTRY_ADDR             15 //R
                                 ...
                                 lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 0)
    {
        ...
        *numDataP = 13;
		...
		(*dataArrayP)[12].id = RES_M_REGISTRY_ADDR;
    }
    ...
    do
    {
		...
		case RES_M_REGISTRY_ADDR:
			//char *addr = getip4addr();
			strcpy(data->registry_addr, getip4addr());
			lwm2m_data_encode_string(data->registry_addr, *dataArrayP + i);
            result = COAP_205_CONTENT;
			break;
