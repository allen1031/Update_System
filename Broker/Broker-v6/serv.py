...
class SP():
	...
	def __Start_LWM2M(self,db_name,collection_name):
		try:
			print "lwm2m is starting"
			self.__lwm2m =subprocess.Popen(['./lwm2mserver', '-4', '-d', db_name, '-b', collection_name], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
	...
	def __Start_LWM2M_Client(self,endpoint,host_ip,db_ip,db_name):
		try:
			...
			self.__lwm2m_client =subprocess.Popen(['./lwm2mclient', '-4', '-h', host_ip, '-d', db_name, '-n', endpoint], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
		except:
			return False
	...
	def ClientUpdate(self,db,collection,broker_key,client_key,group_size,package_url):
		...
		while True:
			...
			while 1:
				...
				if array[Data.container_url] == value :
					break;
				
			exec_result = self.WakaamaExecute(db,collection,broker_key,i, exe)
	...
	def Main_Server_Process(self,db_name,endpoint,host_ip,db_ip):
		...
		self.__InitSuccess = self.__Start_LWM2M(db_name,collection_name)
		self.__InitSuccess_client = self.__Start_LWM2M_Client(endpoint,host_ip,db_ip,db_name) 
